...
public class PainelRede extends JPanel{
   ...
   public void desenhar(RedeNeural rede){
      ...
      yCamadaEntrada = y0 + (altura/2) - (alturaDesenho * (entrada.quantidadeNeuroniosTotal())) + (espacoVerticalEntreNeuronio * (entrada.quantidadeNeuroniosTotal()+2));
      for(int i = 0; i < rede.obterQuantidadeOcultas(); i++){
         yCamadaOculta[i] = y0+(altura/2) - (alturaDesenho * (rede.obterCamadaOculta(i).quantidadeNeuroniosTotal())) + (espacoVerticalEntreNeuronio * (rede.obterCamadaOculta(i).quantidadeNeuroniosTotal()+2));
      }
      yCamadaSaida = y0 + (altura/2) - (alturaDesenho * (saida.quantidadeNeuroniosTotal())) + (espacoVerticalEntreNeuronio * (saida.quantidadeNeuroniosTotal()+1));
   ...
   private void desenharConexoesEntrada(Graphics2D g2){
      ...
      int i, j;
      int qNeuroniosOculta = rede.obterCamadaOculta(0).quantidadeNeuroniosTotal();
   ...
   private void desenharConexoesOcultas(Graphics2D g2){
      ...
      for(i = 0; i < coordOcultas.size()-1; i++){//percorrer ocultas
         ...
         for(j = 0; j < coordOcultas.get(i).size(); j++){//percorrer neuronios da camada oculta atual
            ...
            if(rede.obterCamadaOculta(i+1).temBias()) qNeuroniosProxima = rede.obterCamadaOculta(i+1).quantidadeNeuroniosTotal()-1;
            else qNeuroniosProxima = rede.obterCamadaOculta(i+1).quantidadeNeuroniosTotal();
   ...
   private void desenharCamadaEntrada(Graphics2D g2){
      ...
      coordEntrada.clear();
      for(contador = 0; contador < entrada.quantidadeNeuroniosTotal(); contador++){
         ...
         int xTexto = -30;
         int yTexto = 18;
         if(entrada.temBias() && (contador < entrada.quantidadeNeuroniosTotal()-1)) g2.drawString(("e" + contador), (x+xTexto), (y+yTexto));
         ...
         if(entrada.temBias() && contador == entrada.quantidadeNeuroniosTotal()-1) desenharNeuronioBias(g2, x, y);
   ...
   private void desenharCamadasOcultas(Graphics2D g2){
      ...
      for(contador = 0; contador < rede.obterQuantidadeOcultas(); contador++){//percorrer ocultas
         ...
         for(contador2 = 0; contador2 < rede.obterCamadaOculta(contador).quantidadeNeuroniosTotal(); contador2++){//percorrer neuronios de uma oculta
            ...
            if(rede.obterCamadaOculta(contador).temBias() && contador2 ==  rede.obterCamadaOculta(contador).quantidadeNeuroniosTotal()-1) desenharNeuronioBias(g2, x, y);
   ...
   private void desenharCamadaSaida(Graphics2D g2){
      ...
      Camada saida = rede.obterCamadaSaida();
      coordSaida.clear();
      for(contador = 0; contador < saida.quantidadeNeuroniosTotal(); contador++){
